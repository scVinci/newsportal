    ...
    <div class="bg-light rounded p-4">
        ...
        <form action="{{route('admin.posts.store')}}" method="post" enctype="multipart/form-data">
                ...
                <select class="form-select" id="category_id" aria-label="Floating label select example" name="category_id">
                    ...
                    @foreach($categories as $item)
                        <option value="{{$item->id}}"
                            {{old('category_id') && old('category_id') == $item->id?'selected':false }}
                        >{{$item->title}}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control" placeholder="Text here" id="floatingTextarea" style="height: 350px;" name="text">{{old('text')?old('text'):false}}</textarea>
            </div>
            <div class="mb-3">
